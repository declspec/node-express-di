...
function $RouteProvider() {
    ...
    this.error = function(fn) {
        if ("function" !== typeof(fn) || fn.length < 4)
            routerErr("badrt", "Invalid error handler provided. Expected a function that matches function(err, req, res, next).");
    ...
    this.$get = [ "$q", "$controller", "$filter", "$injector", function($q, $controller, $filter, $injector) {
        ...
        function createRoute(route) {
            ...
            if ("function" === typeof(route) && !isInvokable(route))
                return route;
            ...
            return function(req, res, next) {
                ...
                $q.when(promise).then(function(handled) {
                    // Filter handled the request, no need to continue
                    if (handled) 
                        return;
                    
                    // Need to resolve locals first
                    var locals = { req: req, res: res, next: next };
                    if ("object" === typeof(route.resolve))
                        _.assign(locals, route.resolve);
                    
                    $q.all(locals).then(function(results) {
                        return isInvokable(action)
                            ? $injector.invoke(action, null, results)
                            : action(req, res, next);
                    });
                });
            };
