...
function $RouteProvider() {
    ...
    // Middleware handler
    this.use = function(route) {
        routes.push(createRouteConfig(null, "use", route));
        return
    };
    ...
    this.$get = [ "$q", "$controller", "$filter", "$injector", "$resolve", "$express", function($q, $controller, $filter, $injector, $resolve, $express) {
        ...
        return {
            resolve: function() {
                ...
                for(i = 0, j = routes.length; i < j; ++i) {
                    var route = routes[i],
                        handler = createStandardRoute(route);
                    addExpressRoute(route.method, route.path, handler);
                }
                
                // Create the 'otherwise' route if it exists
                if (otherwise)
                    addExpressRoute(otherwise.method, otherwise.path, createStandardRoute(otherwise));
        ...
        function addExpressRoute(methodName, path, handler) {
            var method = $express[methodName.toLowerCase()];
            if (path) method.call($express, path, handler)
            else method.call($express, handler);
        }
        ...
        function handleRequest(route, err, req, res, next) {
            ...
            if (isInvocable(route.action))
                ...
                promise = filter && filter(req, res, route.filterArg);
            }
        ...
        function getFilter(filterArg, routePath) {
            ...
            var filter = isInvocable(filterArg)
    ...
    function isInvocable(expr) {
        return expr && (Array.isArray(expr) || ("function" === typeof(expr) && (expr.length === 0 || "$inject" in expr)));
    }
